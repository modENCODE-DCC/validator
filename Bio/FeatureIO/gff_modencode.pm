...
Bio::FeatureIO::gff_modencode - read/write GFF feature files
...
# Extension of Bio::SeqFeature::Annotated to allow features to contain features
# larger than themselves
package Bio::SeqFeature::Annotated_UnrestrictedChildren;

use base qw(Bio::SeqFeature::Annotated);
...
package Bio::FeatureIO::gff_modencode;
use strict;
...
=head2 next_feature()
 ...
 Returns : a Bio::SeqFeature::Annotated_UnrestrictedChildren object
 Args    : N/A
...
=head2 next_feature_group
 ...
 Returns : an array of Bio::SeqFeature::Annotated_UnrestrictedChildren objects
 Args    : none
...
=head2 write_feature()
 ...
 Usage   : $featureio->write_feature( Bio::SeqFeature::Annotated_UnrestrictedChildren->new(...) );
 Function: writes a feature in GFF format.  the GFF version used is governed by the
           ...
           version 3.
 Returns : ###FIXME
 Args    : a Bio::SeqFeature::Annotated_UnrestrictedChildren object.
...
sub write_feature {
  ...
  $self->throw("only Bio::SeqFeature::Annotated_UnrestrictedChildren objects are writeable") unless $feature->isa('Bio::SeqFeature::Annotated_UnrestrictedChildren');
...
#	    $self->_buffer_feature($f);
  ...
  # Patch/extend Bio::FeatureIO::gff::_handle_directive to allow organism specification
  elsif($directive eq 'organism'){
    my ($region, $genus, @species) = @arg;
    if (!$self->sequence_region($region)) {
      $self->throw("Can't use the \"organism\" directive unless a sequence-region directive is specified before it");
    } else {
      my $genus_anno = new Bio::Annotation::SimpleValue(-value => $genus);
      my $species_anno = new Bio::Annotation::SimpleValue(-value => join(" ", @species));
      $self->sequence_region($region)->add_Annotation("Organism_Genus", $genus_anno);
      $self->sequence_region($region)->add_Annotation("Organism_Species", $species_anno);
    }
  }
...
Bio::SeqFeature::Annotated_UnrestrictedChildren object.
...
sub _handle_feature {
  ...
  my $feat = Bio::SeqFeature::Annotated_UnrestrictedChildren->new();
 ...
 grep {/^[A-Z]/} keys %attr;
  ...
  if($attr{ID}){
    ...
    if (($feat->get_Annotations('ID'))[0]->value() eq $feat->seq_id()) {
      ...
      $seq_f->add_Annotation(($feat->get_Annotations('ID'))[0]);
...
=head2 _handle_non_reserved_tag()
 ...
 Returns : An updated Bio::SeqFeature::Annotated_UnrestrictedChildren object
 Args    : A Bio::SeqFeature::Annotated_UnrestrictedChildren and a tag/value pair
