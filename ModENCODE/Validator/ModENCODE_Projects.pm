...
sub validate {
  ...
  my ($group) = grep { $_->get_object->get_name() eq "Project" } $experiment->get_properties();
  my ($subgroup) = grep { $_->get_object->get_name() eq "Lab" } $experiment->get_properties();

  # GROUP
  my $group_name = $group->get_object->get_value() if $group;
  $group_name =~ s/^\s*|\s*$//g;
  ...
  $group->get_object->set_value($matching_group_name);

  # SUBGROUP
  if (!$subgroup || !length($subgroup->get_object->get_value())) {
    log_error "No modENCODE project sub-group defined - defaulting to main group '$matching_group_name'.", "warning";
    $subgroup = new ModENCODE::Chado::ExperimentProp({
        'name' => 'Lab',
        'value' => $group->get_object->get_value,
        'termsource' => $group->get_object->get_termsource,
        'experiment' => $experiment,
        'type' => $group->get_object->get_type,
      });
    $experiment->add_property($subgroup);
  }
  my $subgroup_name = $subgroup->get_object->get_value();
  $subgroup_name =~ s/^\s*|\s*$//g;
