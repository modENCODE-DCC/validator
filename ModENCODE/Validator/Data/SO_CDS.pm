package ModENCODE::Validator::Data::SO_CDS;
=pod
...
sub validate {
  ...
  log_error "Loading CDSs from Chado database(s).", "notice", ">";

  foreach my $parse (
    ['modENCODE', $self->get_modencode_chado_parser(), [ 'ModEncode Annotation IDs' ]],
    ['FlyBase', $self->get_flybase_chado_parser(), [ 'FlyBase Annotation IDs' ]],
    ['WormBase',  $self->get_wormbase_chado_parser(), [ 'Wormbase IDs' ]],
  ) {
    my ($parser_name, $parser, $dbnames) = @$parse;
    if (!$parser) {
      log_error "Can't check CDSs against the $parser_name database; skipping.", "warning";
      next;
    }

    log_error "Checking for CDSs in the $parser_name database.", "notice", ">";
    ...
    while (my $ap_datum = $self->next_datum) {
      ...
      log_error "Trying to find CDS by $accession in $parser_name.", "debug";

      if (!length($accession)) {
        log_error "Empty value for CDS accession in column " . $datum_obj->get_heading . " [" . $datum_obj->get_name . "].", "warning";
      ...
      if ($feature->get_object->get_type(1)->get_name ne "CDS") {
        log_error "Found a feature for $accession in $parser_name, but it is a " . $feature->get_object->get_type(1)->get_name . ", not a CDS. Skipping.", "warning";
        next;
      }
      ...
      log_error "Found CDS $accession in $parser_name.", "notice";
  ...
  if ($self->num_data) {
    ...
    $success = 0;
    log_error "Couldn't find CDSs (" . join(", ", sort(@accessions)) . ") in any database.", "error";
  }
