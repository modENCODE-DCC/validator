package ModENCODE::Validator::Data::AE_lite;
=pod

=head1 NAME

ModENCODE::Validator::Data::GEO_lite - Class for validating and updating
BIR-TAB L<Data|ModENCODE::Chado::Data> objects containing EBI accessions for
...
accession. This is a temporary standin until ArrayExpress records can be properly fetched from EBI repository.
...
To use this validator in a standalone way:
  ...
  my $validator = new ModENCODE::Validator::Data::AE_lite();
...
sub validate {
  ...
  log_error "Validating presence of " . $self->num_data . " ArrayExpress submission(s).", "notice", ">";
  ...
  foreach my $parse (
    ...
    log_error "Checking for " . $self->num_data . " ArrayExpress accessions already in the $parser_name database...", "notice", ">";
    while (my $ap_datum = $self->next_datum) {
      ...
      if (!length($accession)) {
        log_error "Empty value for ArrayExpress accession in column " . $datum_obj->get_heading . " [" . $datum_obj->get_name . "].", "warning";
      ...
      if ($cvterm ne "ArrayExpress_record") {
        log_error "Found a feature in $parser_name, but it is of type '$cv:$cvterm', not 'ME:ArrayExpress_record'. Not using it.", "warning";
        next;
      }
  ...
  log_error "Looking up " . $self->num_data . " accessions in ArrayExpress " , "notice", ">";
  ...
  while ($self->num_data) {
	...
	log_error ("Fetching AE ids: " .  $fetch_query . ".", "notice", ">");
    } else {
    log_error "Fetching batch of " . scalar(@batch_query) . " ArrayExpress accessions.", "notice", ">";
    }
    ...
    if ($done > MAX_TRIES) {
      ...
      log_error "Couldn't fetch AE accessions by ID after " . MAX_TRIES . " tries.", "warning";
...
sub handle_summary_results {
  ...
  foreach my $ap_datum (@ap_data) {
    ...
    my $url = "http://www.ebi.ac.uk/microarray-as/ae/browse.html?keywords=" . $genbank_acc;
