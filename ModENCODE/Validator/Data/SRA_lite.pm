package ModENCODE::Validator::Data::SRA_lite;
=pod
...
sub validate {
  ...
  log_error "Validating presence of " . $self->num_data . " SRA submission(s).", "notice", ">";
  ...
  foreach my $parse (
    ...
    log_error "Checking for " . $self->num_data . " SRA accessions already in the $parser_name database...", "notice", ">";
  ...
  log_error "Looking up " . $self->num_data . " accessions in SRA " , "notice", ">";
  ...
  while ($self->num_data) {
    ...
    log_error "Fetching batch of " . scalar(@batch_query) . " SRA accessions.", "notice", ">";
    }
    ...
    while ($done++ < MAX_TRIES) {
      log_error "Searching for remaining batch of " . scalar(@batch_query) . " SRA accessions.", "notice";
...
sub handle_summary_results {
  ...
  foreach my $ap_datum (@ap_data) {
    ...
    #can't handle SRA ids in eutils - simply create url reference to record

    my $genbank_acc = $datum_obj->get_value();

    my $url = "http://www.ncbi.nlm.nih.gov/sites/entrez?db=sra&report=full";
    if (($genbank_acc =~ /^SR[AXRS]\d+/) || ($genbank_acc =~ /TMPID/)) {
