package ModENCODE::Validator::Data::SO_protein;
=pod

=head1 NAME

ModENCODE::Validator::Data::SO_protein - Class for validating and updating
BIR-TAB L<Data|ModENCODE::Chado::Data> objects containing protein names to
include L<Features|ModENCODE::Chado::Feature> for those proteins.
...
L<DBXrefs|ModENCODE::Chado::DBXref> for a provided protein name (as
...
When given L<ModENCODE::Chado::Data> objects with values that are protein
names, this modules uses
L<ModENCODE::Parser::Chado/get_feature_id_by_name_and_type($name, $type,
$allow_isa)> to pull out C<feature_id>s for features of type C<SO:protein> or
children of the C<SO:protein> type like C<SO:mRNA>. (This is achieved by
...
To use this validator in a standalone way:
  ...
  my $validator = new ModENCODE::Validator::Data::SO_protein();
...
$applied_protocol)> have values that exist as protein names accession in the
...
that datum with a newly attached feature based on a protein record and other
...
with the name equal to the protein name - if these are found, they are
...
sub validate {
  ...
  log_error "Loading proteins from Chado database(s).", "notice", ">";
  ...
  foreach my $parse (
    ...
    if (!$parser) {
      log_error "Can't check proteins against the $parser_name database; skipping.", "warning";
      next;
    }

    log_error "Checking for proteins in the $parser_name database.", "notice";
    ...
    while (my $ap_datum = $self->next_datum) {
      ...
      if ($feature->get_object->get_type(1)->get_name ne "protein") {
        log_error "Found a feature for $accession in $parser_name, but it is a " . $feature->get_object->get_type(1)->get_name . ", not a protein. Skipping.", "warning";
        next;
      }
      ...
      log_error "Found protein $accession in $parser_name.", "debug";
      $datum->get_object->add_feature($feature);
    }
    $self->rewind();
  }
  if ($self->num_data) {
    # Some proteins weren't found in any parser
    my @accessions;
    while (my $ap_datum = $self->next_datum) {
      my ($applied_protocol, $direction, $datum) = @$ap_datum;
    ...
    $success = 0;
    log_error "Couldn't find proteins (" . join(", ", sort(@accessions)) . ") in any database.", "error";
  }
