...
sub get_srcfeature_id {
  my $self = shift;
  return $srcfeature{ident $self} ? $srcfeature{ident $self}->get_id : undef;
}

sub get_srcfeature {
  my $self = shift;
  my $get_cached_object = shift || 0;
  my $srcfeature = $srcfeature{ident $self};
  return undef unless defined $srcfeature;
  return $get_cached_object ? $srcfeature->get_object : $srcfeature;
}

sub set_srcfeature {
  my ($self, $srcfeature) = @_;
  ($srcfeature->get_object->isa('ModENCODE::Chado::Feature')) or Carp::confess("Can't add a " . ref($srcfeature) . " as a srcfeature.");
...
sub to_string {
  ...
  $string .= " " . $self->get_srcfeature->get_object->get_name() if $self->get_srcfeature();
